      ...
      <div onClick={e=>e.stopPropagation()} className="w-full max-w-[520px] rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 shadow-xl">
        ...
        <div className="mt-4 space-y-4">
          <div className="p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 text-xs leading-5">
            <b>No keys shipped. No storage on our servers.</b> Your Gemini API key stays in <code>localStorage</code> on your device and is sent directly to Google. Remove anytime. Google Sign-In is optional — just for consent & personalization.
          </div>

          <div>
            <div className="text-sm font-semibold">Google Sign-In (optional)</div>
            <div className="text-xs text-zinc-500 mt-1">Sign in to consent to AI usage. We only store name/photo locally.</div>
            <div className="mt-3">
              {user ? (
                ...
                <div className="text-xs p-3 rounded-xl border border-dashed">Set <code>VITE_GOOGLE_CLIENT_ID</code> in <code>.env</code> to enable Google Sign-In. Or skip — AI still works with just the API key below.</div>
              )}
            </div>
          </div>

          <div>
            <div className="text-sm font-semibold">Gemini API Key (BYO)</div>
            <div className="text-xs text-zinc-500 mt-1">Get free key at <a href="https://aistudio.google.com/app/apikey" target="_blank" className="underline text-[#0A66C2]">aistudio.google.com/app/apikey</a> • Stored locally only</div>
