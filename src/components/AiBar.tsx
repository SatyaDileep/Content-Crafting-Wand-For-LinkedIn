...
function Btn({ a, label, loading, onClick }: { a: GeminiAction; label: string; loading: GeminiAction | null; onClick: (a: GeminiAction)=>void }) {
  return (
    <button disabled={!!loading} onClick={()=>onClick(a)} className="px-3 py-1.5 rounded-full border text-xs font-medium bg-white dark:bg-zinc-800 hover:bg-zinc-50 disabled:opacity-50">
      {loading===a ? "…" : label}
    </button>
  )
}
  ...
  return (
    <div className="p-2.5 bg-gradient-to-r from-violet-50 to-blue-50 dark:from-zinc-800 dark:to-zinc-800 border-b border-zinc-100 dark:border-zinc-800">
      <div className="flex flex-wrap gap-1.5">
        <Btn a="improve" label="✨ Improve" loading={loading} onClick={run} />
